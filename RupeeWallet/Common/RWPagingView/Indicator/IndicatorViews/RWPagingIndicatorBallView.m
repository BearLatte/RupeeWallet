//
//  RWPagingIndicatorBallView.m
//  RupeeWallet
//
//  Created by Tim on 2023/7/15.
//

#import "RWPagingIndicatorBallView.h"
#import "RWPagingFactory.h"

@interface RWPagingIndicatorBallView()
@property (nonatomic, strong) UIView *smallBall;
@property (nonatomic, strong) UIView *bigBall;
@property (nonatomic, strong) CAShapeLayer *shapeLayer;
@end

@implementation RWPagingIndicatorBallView
...
- (void)rw_contentScrollViewDidScroll:(RWPagingIndicatorParamsModel *)model {
    ...
    if (percent == 0) {
        ...
        if (percent <= 0.5) {
            targetXOfBigBall = [RWPagingFactory interpolationFrom:leftX to:(rightX - self.ballScrollOffsetX) percent:percent*2];
            targetWidthOfSmallBall = [RWPagingFactory interpolationFrom:ballWidth to:ballWidth/2 percent:percent*2];
        }else {
            targetXOfBigBall = [RWPagingFactory interpolationFrom:(rightX - self.ballScrollOffsetX) to:rightX percent:(percent - 0.5)*2];
            targetWidthOfSmallBall = [RWPagingFactory interpolationFrom:ballWidth/2 to:0 percent:(percent - 0.5)*2];
            targetXOfSmallBall = [RWPagingFactory interpolationFrom:leftX to:rightX percent:(percent - 0.5)*2];
        }
    }
