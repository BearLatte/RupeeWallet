...
@implementation RWPagingTitleCell
...
- (void)layoutSubviews {
    ...
    RWPagingTitleCellModel *myCellModel = (RWPagingTitleCellModel *)self.cellModel;
    switch (myCellModel.titleLabelAnchorPointStyle) {
        case RWPagingTitleLabelAnchorPointStyleCenter: {
        ...
        case RWPagingTitleLabelAnchorPointStyleTop: {
        ...
        case RWPagingTitleLabelAnchorPointStyleBottom: {
...
- (void)reloadData:(RWPagingBaseCellModel *)cellModel {
    ...
    RWPagingTitleCellModel *myCellModel = (RWPagingTitleCellModel *)cellModel;
    ...
    if (myCellModel.isTitleLabelStrokeWidthEnabled) {
        if (myCellModel.isSelectedAnimationEnabled && [self checkCanStartSelectedAnimation:myCellModel]) {
            RWPagingCellSelectedAnimationBlock block = [self preferredTitleStrokeWidthAnimationBlock:myCellModel attributedString:attributedString];
            [self addSelectedAnimationBlock:block];
        } else {
    ...
    if (myCellModel.isTitleLabelMaskEnabled) {
        ...
        self.titleLabel.layer.mask = nil;
        if (myCellModel.isSelectedAnimationEnabled && [self checkCanStartSelectedAnimation:myCellModel]) {
            RWPagingCellSelectedAnimationBlock block = [self preferredTitleColorAnimationBlock:myCellModel];
            [self addSelectedAnimationBlock:block];
        } else {
...
- (RWPagingCellSelectedAnimationBlock)preferredTitleZoomAnimationBlock:(RWPagingTitleCellModel *)cellModel baseScale:(CGFloat)baseScale {
    ...
    return ^(CGFloat percent) {
        if (cellModel.isSelected) {
            //将要选中，scale从小到大插值渐变
            cellModel.titleLabelCurrentZoomScale = [RWPagingFactory interpolationFrom:cellModel.titleLabelNormalZoomScale to:cellModel.titleLabelSelectedZoomScale percent:percent];
        } else {
            //将要取消选中，scale从大到小插值渐变
            cellModel.titleLabelCurrentZoomScale = [RWPagingFactory interpolationFrom:cellModel.titleLabelSelectedZoomScale to:cellModel.titleLabelNormalZoomScale percent:percent];
        }
...
- (RWPagingCellSelectedAnimationBlock)preferredTitleStrokeWidthAnimationBlock:(RWPagingTitleCellModel *)cellModel attributedString:(NSMutableAttributedString *)attributedString {
    ...
    return ^(CGFloat percent) {
        if (cellModel.isSelected) {
            ...
            cellModel.titleLabelCurrentStrokeWidth = [RWPagingFactory interpolationFrom:cellModel.titleLabelNormalStrokeWidth to:cellModel.titleLabelSelectedStrokeWidth percent:percent];
        } else {
            //将要取消选中，StrokeWidth从大到小插值渐变
            cellModel.titleLabelCurrentStrokeWidth = [RWPagingFactory interpolationFrom:cellModel.titleLabelSelectedStrokeWidth to:cellModel.titleLabelNormalStrokeWidth percent:percent];
        }
...
- (RWPagingCellSelectedAnimationBlock)preferredTitleColorAnimationBlock:(RWPagingTitleCellModel *)cellModel {
    ...
    return ^(CGFloat percent) {
        if (cellModel.isSelected) {
            ...
            cellModel.titleCurrentColor = [RWPagingFactory interpolationColorFrom:cellModel.titleNormalColor to:cellModel.titleSelectedColor percent:percent];
        } else {
            //将要取消选中，textColor从titleSelectedColor到titleNormalColor插值渐变
            cellModel.titleCurrentColor = [RWPagingFactory interpolationColorFrom:cellModel.titleSelectedColor to:cellModel.titleNormalColor percent:percent];
        }
