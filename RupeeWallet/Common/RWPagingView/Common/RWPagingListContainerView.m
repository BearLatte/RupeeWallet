...
@interface RWPagingListContainerViewController : UIViewController
...
@implementation RWPagingListContainerViewController
- (void)dealloc
{
...
@interface RWPagingListContainerView () <UIScrollViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource>
@property (nonatomic, weak) id<RWPagingListContainerViewDelegate> delegate;
@property (nonatomic, strong) UIScrollView *scrollView;
@property (nonatomic, assign) NSInteger currentIndex;
@property (nonatomic, strong) NSMutableDictionary <NSNumber *, id<RWPagingListContentViewDelegate>> *validListDict;
@property (nonatomic, assign) NSInteger willAppearIndex;
@property (nonatomic, assign) NSInteger willDisappearIndex;
@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) RWPagingListContainerViewController *containerVC;
@end

@implementation RWPagingListContainerView

- (instancetype)initWithType:(RWPagingListContainerType)type delegate:(id<RWPagingListContainerViewDelegate>)delegate{
...
- (void)initializeViews {
    ...
    _containerVC = [[RWPagingListContainerViewController alloc] init];
    ...
    if (self.containerType == RWPagingListContainerType_ScrollView) {
        if (self.delegate &&
...
- (void)layoutSubviews {
    ...
    if (self.containerType == RWPagingListContainerType_ScrollView) {
        if (CGRectEqualToRect(self.scrollView.frame, CGRectZero) ||  !CGSizeEqualToSize(self.scrollView.bounds.size, self.bounds.size)) {
            ...
            [_validListDict enumerateKeysAndObjectsUsingBlock:^(NSNumber * _Nonnull index, id<RWPagingListContentViewDelegate>  _Nonnull list, BOOL * _Nonnull stop) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UIView* listView = nil;
    id<RWPagingListContentViewDelegate> list = _validListDict[@(indexPath.item)];
    if (list != nil) {
...
#pragma mark - RWPagingViewListContainer
...
- (void)reloadData {
    for (id<RWPagingListContentViewDelegate> list in _validListDict.allValues) {
...
- (void)initListIfNeededAtIndex:(NSInteger)index {
    ...
    id<RWPagingListContentViewDelegate> list = _validListDict[@(index)];
    if (list != nil) {
        return;
    }
